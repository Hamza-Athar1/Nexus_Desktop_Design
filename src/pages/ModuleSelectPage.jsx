...
export default function ModuleSelectPage() {
  ...
  const handleLaunch = () => { if (active) setLaunched(true); };

  return (
    <div className="min-h-screen w-full flex flex-col nexus-bg">

      {/* ── Navbar ──────────────────────────────────────────────────── */}
      <header className="w-full nexus-navbar flex items-center justify-between px-7 h-[58px] flex-shrink-0 shadow-[0_2px_16px_rgba(0,0,0,0.2)]">
        {/* Brand */}
        <div className="flex items-center gap-2.5">
          <NexusLogo size={34} variant="light" />
          <span className="text-white font-bold text-[15px] tracking-[0.3px]">Nexus Desktop</span>
        </div>

        {/* Right */}
        <div className="flex items-center gap-4">
          <span className="text-[12px] text-white/60">Choose a module to continue</span>
          <button
            id="logout-btn"
            onClick={() => navigate('/')}
            className="flex items-center gap-1.5 bg-white/[0.12] border border-white/20 rounded-lg text-white text-[13px] font-medium px-3.5 py-1.5 cursor-pointer hover:bg-white/20 transition-all"
          >
            <LogOut size={14} />
            Logout
          </button>
      ...
      {/* ── Main ────────────────────────────────────────────────────── */}
      <main className="flex-1 flex flex-col items-center px-8 pt-12 pb-10 gap-9">

        {/* Page title */}
        <div className="text-center">
          <h1 className="text-[26px] font-extrabold text-[#1a2e1a] mb-1.5 tracking-tight">
            Choose Your Business Module
          </h1>
          <p className="text-[13.5px] text-[#4a6a3a]">
          ...
          {MODULES.map(mod => {
            ...
            return (
              <button
                ...
                className={[
                  'relative flex flex-col items-center gap-2.5 px-4 pt-6 pb-5 rounded-2xl',
              ...
              >
                ...
                {/* Icon bubble */}
                <span
                  className="flex items-center justify-center w-[58px] h-[58px] rounded-2xl text-[28px] transition-colors"
                ...
                {/* Description */}
                <span className="text-[11px] text-gray-500 text-center leading-relaxed">
                  {mod.desc}
                </span>
