...
export default function MessageOwnerModal({ shop, onClose, onSend }) {
  ...
  useEffect(() => {
    if (shop) { setSubject('Regarding your billing update'); setBody(''); setSent(false); }
  }, [shop]);
      ...
      <div className="relative bg-[#efeacb] rounded-[24px] border border-[#bfbc9b] p-8 w-full max-w-[440px] shadow-2xl flex flex-col text-[#152f16] gap-5">
        ...
        {sent ? (
          ...
          <div className="flex flex-col items-center gap-5 py-4 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#d5eedb] text-[#137333]">
              <CheckCircle size={36} strokeWidth={2} />
            </div>
            <div>
              <h3 className="text-2xl font-bold font-serif text-[#152f16] leading-tight">Message sent!</h3>
              <p className="text-sm font-semibold text-[#55694a] mt-2 leading-relaxed">
                Your message to <span className="text-[#0d3b1b] font-bold">{shop.owner}</span> at{' '}
                <span className="text-[#0d3b1b] font-bold">{shop.business}</span> was delivered successfully.
              </p>
            </div>
            <button
              type="button"
              onClick={onClose}
              className="w-full py-3.5 bg-[#0d3b1b] text-[#efeacb] text-base font-bold rounded-xl hover:bg-[#072410] transition cursor-pointer"
            >
              Done
            </button>
          </div>
