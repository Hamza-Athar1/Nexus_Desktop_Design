 ...
 *  onCancel        ()=>void
 *  onCreatePalette ({name,colorPrimary,colorAccent,colorShade,colorLight}) => Promise<palette|null>
...
export default function AddPOSModal({ isOpen, palettes, onCancel, onCreatePalette, _onDeletePalette, onSave }) {
  ...
  // Auto-select first palette when list loads
  const resolvedTheme = theme ?? palettes?.[0] ?? null;

  if (!isOpen) return null;

  const handleAddPalette = async (formData) => {
    // formData from AddPaletteForm: { name, colors:[primary,accent,shade,deep] }
    // colors[0]=primary, colors[1]=accent, colors[2]=shade(light in form), colors[3]=deep(light)
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    await onSave({ name: name.trim(), priceCents, paletteId: resolvedTheme?.id ?? null });
    setSaving(false);
    setName(''); setPrice(''); setTheme(null);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3 sm:p-4 font-sans select-none text-[#14391a]">
      <div className="bg-[#faf8ed] border border-[#14391a]/15 rounded-[24px] w-full max-w-4xl p-4 sm:p-6.5 flex flex-col gap-5 shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between border-b border-[#14391a]/10 pb-3">
          <h2 className="text-[22px] font-black text-[#14391a]">Add new POS</h2>
            ...
            <div className="lg:col-span-7 flex flex-col gap-6">
              ...
              <div className="flex flex-wrap sm:flex-nowrap gap-4">
                <input type="text" required value={name} onChange={e => setName(e.target.value)}
                  placeholder="POS name e.g. Salon"
