...
export default function AdminSidebar({ activeTab, isOpen, onClose }) {
  ...
  return (
    <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-72 flex flex-col shrink-0 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
      {/* Beige Logo Area */}
      <div className="bg-[#efeacb] p-6 flex flex-col items-start justify-center border-b border-[#c8c2a3]/30 h-40 shrink-0 relative">
        <img src="/Nexus_superadmin.png" alt="Nexus Logo" className="h-48 w-auto object-contain" />
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="lg:hidden absolute top-4 right-4 text-[#0c3818] hover:text-black cursor-pointer"
          >
            <X size={20} />
          </button>
        )}
      ...
      <div className="bg-[#0c3818] flex-1 p-5 flex flex-col gap-6 overflow-y-auto">
        <div className="flex flex-col gap-2 items-start">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center justify-center w-10 h-10 rounded-lg border border-[#3f5a3b] bg-[#082813] text-white hover:bg-[#114720] transition duration-200 cursor-pointer"
          >
            <ArrowLeft size={18} />
          </button>
        </div>
